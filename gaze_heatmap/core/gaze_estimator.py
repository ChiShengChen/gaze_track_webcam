...
class GazeEstimator:
    """
    Wrapper for gaze estimation using ptgaze (preferred) or MediaPipe fallback.
    
    Supports both ETH-XGaze and MPIIGaze models when ptgaze is available.
    Falls back to MediaPipe iris tracking when ptgaze is not installed.
    """
    ...
    def __init__(self, device: str = 'cpu', model: str = 'eth-xgaze'):
        ...
        Args:
            ...
            model: 'eth-xgaze' (recommended) or 'mpiigaze'
        """
        self.device = device
        self.model = model
        self.use_ptgaze = PTGAZE_AVAILABLE
    ...
    def close(self):
        """Release resources."""
        if not self.use_ptgaze and hasattr(self, 'face_mesh'):
